...
def create_browser_agent(
    ...
    if not organization_id:
        raise ValueError("organization_id is required to create browser agent")
    
    logger.info("[BROWSER] Creating lazy agent for org: %s (llm_config_id: %s)", organization_id, llm_config_id)
    ...
    if sub_agent_config:
        ...
        if enabled_subs:
            ...
            logger.info("[BROWSER] Configured with sub-agents: %s", enabled_subs)
